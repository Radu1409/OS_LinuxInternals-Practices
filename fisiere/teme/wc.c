...
int main(int argc, char** argv)
{
    ...
    if (numFiles > 0)
    {
        ...
        sizes = malloc(sizeof(size_t) * numFields);
	
    // go through all the files
    for(i=(numFiles > 0) ? optind : 0, file=(numFiles > 0) ? files : NULL, size=sizes; i<argc; i++, (numFiles > 0) ? file++ : NULL)
    {
        int byte_count=0;
        int word_count = 0;
        int line_count = 0;
    ...
    {
        if (numFiles < 1)
            width = snprintf(NULL, 0, "%d", byte_num);
        else
	{
