...
int main(int argc, char **argv)
{
    ...
    if(!start_from && ((line_number == 0 && nFlag) || (char_number == 0 && cFlag)))
        exit(0);
...
void write_from_stdin()
{
    ...
    if(start_from) {
        while((read(STDIN_FILENO,buf,1)) > 0)
        {
            ...
            // start printing
            if((nFlag && line_number<=1) || (cFlag && char_number<=1))
            {
                if(write(STDOUT_FILENO,buf,1) < 0)
                    err_sys("write error");
            }
            else if(start_print && ((nFlag && (counter >= line_number-1)) ||
                               (cFlag && (byte_counter >= char_number-1))))
            {
                if(write(STDOUT_FILENO,buf,1) < 0)
                    err_sys("write error");
            }

            // set start_print when count for bytes/lines reaches its previous byte/line
            if(!start_print && (counter == line_number-1 || byte_counter == char_number-1))
                start_print = 1;
