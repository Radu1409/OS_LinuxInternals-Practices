...
int main(int argc, char* argv[])
{
    ...
    // Obtinerea statusului fisierului sursa
    if((status_src = stat(argv[optind], &id_src)) == -1)
    {
        err_ret("cannot stat '%s'",argv[optind]);
        exit(1);	
    }
    // Deschiderea fisierului sursa
    if((srcFD = open(argv[optind],O_RDONLY)) == -1)
    {
        err_ret("cannot open '%s' for reading",argv[optind]);
        exit(1);	
    }
    ...
    if((stat(argv[argc-1], &dir)) == 0)
    {
    	...
    	if(S_ISDIR(dir.st_mode))
    	{
            ...
            if(argv[argc-1][0] != '/')
                snprintf(destinatie,PATH_MAX,"%s/%s/%s",".",argv[argc-1],argv[optind]);
            else
            {
